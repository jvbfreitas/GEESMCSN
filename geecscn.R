geecscn = function(y,mu,sigma2,gama,n,p,alpha,K,Na,x,iter.max, alpha.fix, t, id,
                   corstr = "independence",
  ...
  while((count <= iter.max)){
    ...
    parmse = nlminb(c(nu[1],nu[2],sigma2,gama),nllg3, y = y, mu = mu0,
                    lower = c(0.01,0.1,0.1,-0.9),
                    upper = c(0.9,0.9,10,0.9))$par #####
    nu  = c(parmse[1],parmse[2])
    s20 = parmse[3]
    gama0 = parmse[4]
  ...
  return(list(beta = beta, theta = theta, sigma2 = sigma2, gama = gama, mu = mu,
              ...
              vareps = vareps, varnaive = varnaive, df = df, vary = vary))
}
