geecss = function(y,mu,sigma2,gama,n,p,alpha,K,Na,x,iter.max, alpha.fix, t, id,
                  corstr = "independence",
                  linkmu = "identity",idvec,alphalim = c(0.0001,1000)){
  nllg  <- function(nut,y,mu,sigma2,gama){
    return(-sum(log(dCSS(y, mu, sigma2, gama, 1/nut))))
  }
  ...
  nut = optim(par = 0.15, fn = nllg, y = y, mu = mu, sigma2 = sigma2, gama = gama,
              method = "L-BFGS-B", lower = 0.05, upper = 0.99)$par #####
  ...
  while((count <= iter.max)){
    ...
    parmse = nlminb(c(nut,sigma2,gama),nllg3, y = y, mu = mu0,
                    lower = c(0.05,0.1,-0.9), upper = c(0.99,10,0.9))$par #####
    nut  = parmse[1]
    s20 = parmse[2]
    gama0 = parmse[3]
    nu = 1/nut
    vary0 = s20*nu/(nu-1)
    if(alpha.fix == F){
      ft = function(alpha0) QBICsmn(alpha0,gama0,y,mu0,vary0,x,Na,W,K,"CSS")
      alpha = optim(alpha, ft, method = "L-BFGS-B", lower = alphalim[1],
                    upper = alphalim[2])$par
      print(alpha)
    }
    count <- (count+1)
  ...
  vary = sigma2*nu/(nu-1)
  varu = vary
  Omega = as.matrix(diag(rep(sqrt(varu),N),N)%*%Rm%*%diag(rep(sqrt(varu),N),N))
  ...
  resq = 0
  for(i in 1:N){
    resq[i] = qnorm(pCSS(y[i],mu[i],sigma2, gama, nu))
  }
  return(list(beta = beta, theta = theta, sigma2 = sigma2, gama = gama, mu = mu,
              ...
              clusters = t,
              resrob = resid, sdeps = sqrt(diag(vareps)),
              vareps = vareps, varnaive = varnaive, resnai = resnai,M = M, Sigma = Sigma,
              u = u, Lambda = Lambda, W = W, df = df, vary = vary))
}
