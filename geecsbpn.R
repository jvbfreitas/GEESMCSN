geecsbpn = function(y,mu,sigma2,gama,n,p,alpha,K,Na,x,iter.max, alpha.fix,
                    t, id, corstr = "independence",
                    linkmu = "identity",idvec){
  nllg  <- function(nut,y,mu,sigma2,gama){
    nut1 = nut[1]
    nut2 = nut[2]
    return(-sum(log(dCSBPN(y, mu, sigma2, gama, c(1/nut1,1/nut2)))))
  }
  ...
  while((count <= iter.max)){
    ...
    parmse = nlminb(c(nut[1],nut[2],sigma2,gama),nllg3, y = y, mu = mu0,
                    lower = c(0.04,0.04,0.1,-0.9),
                    upper = c(0.99,1,10,0.9))$par #####
