geecsgt = function(y,mu,gama,n,p,alpha,K,Na,x,iter.max, alpha.fix, t, id,
                   corstr = "independence",
                   linkmu = "identity",idvec){
  nllg  <- function(nut,y,mu,gama){
    nut1 = nut[1]
    nut2 = nut[2]
    return(-sum(log(dCSGT(y, mu, gama, c(1/nut1,1/nut2)))))
  }
  ...
  nut = optim(par = c(0.15,0.15), fn = nllg, y = y, mu = mu,
              gama = gama, method = "L-BFGS-B",
              lower = c(0.03,0.03), upper = c(0.49,0.99))$par #####
  if(linkmu == "identity"){
    yvn = y
  }
  if(linkmu == "log"){
  ...
  while((count <= iter.max)){
    ...
    parmse = nlminb(c(nut[1],nut[2],gama),nllg3, y = y, mu = mu0,
                    lower = c(0.05,0.1,-0.9), upper = c(0.49,0.99,0.9))$par #####
    nut  = c(parmse[1],parmse[2])
    gama0 = parmse[3]
    nu = 1/nut

    count <- (count+1)
    dife = sum(abs(c(beta,theta,gama)-c(beta0,theta0,gama0)))
    print(dife)
    beta = beta0
    theta = theta0
    gama = gama0
  ...
  resq = 0
  for(i in 1:N){
    resq[i] = qnorm(pCSGT(y[i],mu[i],gama, nu))
  }
  return(list(beta = beta, theta = theta, gama = gama, mu = mu,
              nu = nu, alpha = alpha, delta = delta, alphac = alphac,pAIC,
              pBIC,Rm = Rm, nobs = N, nclusters = n, Omega = Sigma,
              clusters = t,
              residual = resid, sdeps = sqrt(diag(vareps)),
              vareps = vareps, varnaive = varnaive, resq = resq, df = df,
              vary = vary, W = W))
}
