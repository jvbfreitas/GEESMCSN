geecsggn = function(y,mu,gama,n,p,alpha,K,Na,x,iter.max, alpha.fix,
  ...
  nllg  <- function(nut,y,mu,gama){
    ...
    nut2 = nut[2]
    nut3 = nut[3]
    return(-sum(log(dCSGGN(y, mu, gama, c(1/nut1,1/nut2,1/nut3)))))
  }
  ...
  nut = nlminb(c(0.15,0.15,0.15), nllg, y = y, mu = mu,
              gama = gama, lower = c(0.05,0.05,0.05),
              upper = c(1,1,1))$par #####
  nu = 1/nut
  if(linkmu == "identity"){
    yvn = y
  ...
  while((count <= iter.max)){
    ...
    vary = gamma(nu[1]-1/nu[2])/(nu[3]*gamma(nu[1])) #####
    varu = vary
    uc2 = split(u,id)
    uc = list(NULL)
    for(i in 1:n){
    ...
    parmse = nlminb(c(nut[1],nut[2],nut[3],gama),nllg3, y = y, mu = mu0,
                    lower = c(0.05,0.05,0.05,-0.9),
                    upper = c(1,1,1,0.9))$par #####
    nut  = c(parmse[1],parmse[2],parmse[3])
    gama0 = parmse[4]
    nu = 1/nut
  ...
  return(list(beta = beta, theta = theta, sigma2 = sigma2, gama = gama, mu = mu,
              ...
              vareps = vareps, df = df, vary = vary, varnaive = varnaive))
}
