geegama = function(y,mu,sigma2,n,p,alpha,K,Na,x,iter.max, alpha.fix, t, id,
  ...
  while((count <= iter.max)){
    ...
    if(alpha.fix == F){
      alpha <- nlminb(alpha, AICgee,lower = alphalim[1], upper = alphalim[2],
                      Na = Na, W = W, K = K, sigma2 = sigma2, mu = mu0,
                      p = p)$par
    }
    rij = (y-mu0)/sqrt(mu^2)
